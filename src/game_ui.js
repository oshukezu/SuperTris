// SuperTris 遊戲 UI 與事件輔助模組 (Game UI Helper)
class GameUIHelper {
  static initHUDButtons(game) {
    ...
    document.getElementById('pause-btn')?.addEventListener('click', () => { setDrawer?.classList.add('hidden'); game.togglePause(); });
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) { if (!game.isPaused && !game.isGameOver) game.togglePause(); }
      else if (!game.isPaused && !game.isGameOver) game.requestWakeLock();
    });
    window.addEventListener('blur', () => { if (!game.isPaused && !game.isGameOver) game.togglePause(); });
  }
